...
def create_academic_year(
    ...
    if current_user.role != RoleEnum.admin:
        raise HTTPException(
            status_code=403, detail="Only admins can create academic years"
        )
    return crud_create_ay(db, ay, tenant_id=current_user.tenant_id)
...
def list_academic_years(
    ...
    return get_academic_years(db, tenant_id=current_user.tenant_id)
...
def get_academic_year(
    ...
    ay = crud_get_ay(db, ay_id, tenant_id=current_user.tenant_id)
    if ay is None:
...
def update_academic_year(
    ...
    if current_user.role != RoleEnum.admin:
        raise HTTPException(
            status_code=403, detail="Only admins can update academic years"
        )
    updated = crud_update_ay(db, ay_id, ay, tenant_id=current_user.tenant_id)
    if updated is None:
        raise HTTPException(status_code=404, detail="Academic Year not found")
    return updated
...
def delete_academic_year(
    ...
    if current_user.role != RoleEnum.admin:
        raise HTTPException(
            status_code=403, detail="Only admins can delete academic years"
        )
    deleted = crud_delete_ay(db, ay_id, tenant_id=current_user.tenant_id)
    if deleted is None:
        raise HTTPException(status_code=404, detail="Academic Year not found")
    return deleted
