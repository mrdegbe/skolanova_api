...
def create_class(
    ...
    return cc(db, class_, tenant_id=current_user.tenant_id)
...
def get_classes(
    ...
    # Uncomment below if you want only admins to list classes
    # if current_user.role != RoleEnum.admin:
    #     raise HTTPException(status_code=403, detail="Only admins can get classes")
    return gcs(db, tenant_id=current_user.tenant_id, skip=skip, limit=limit)
...
def get_class(
    ...
    return gc(db, class_id, tenant_id=current_user.tenant_id)
...
def update_class(
    ...
    return uc(db, class_id, class_, tenant_id=current_user.tenant_id)
...
def delete_class(
    ...
    return dc(db, class_id, tenant_id=current_user.tenant_id)
