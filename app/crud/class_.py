...
def create_class(db: Session, class_: ClassCreate, tenant_id: int):
    db_class = Class(**class_.model_dump(), tenant_id=tenant_id)
    db.add(db_class)
    db.commit()
    db.refresh(db_class)
    return db_class
...
def get_classes(db: Session, tenant_id: int, skip: int = 0, limit: int = 100):
    classes = (
        db.query(Class)
...
def get_class(db: Session, class_id: int, tenant_id: int):
    class_ = (
        db.query(Class)
...
def update_class(db: Session, class_id: int, class_: ClassUpdate, tenant_id: int):
    db_class = (
        db.query(Class)
        .filter(Class.id == class_id, Class.tenant_id == tenant_id)
        .first()
    ...
    if class_.academic_year_id is not None:
...
def delete_class(db: Session, class_id: int, tenant_id: int):
    db_class = (
        db.query(Class)
        .filter(Class.id == class_id, Class.tenant_id == tenant_id)
        .first()
